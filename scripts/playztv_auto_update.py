    ...
    try:
        ...
        for ev in events:
            ...
            for i, (stream, ok, msg) in enumerate(ev.get('_verified', [])):
                ...
                url = stream.get('link') if isinstance(stream, dict) else stream
                csv_lines.append(f'"{ev.get("category","")}","{ev.get("eventName","")}","{ev.get("teamAName","")}","{ev.get("teamBName","")}","{ev.get("date","")}","{ev.get("time","")}","{label}","{url}",{ok},"{msg}"')
        ...
        for fname in ['playztv_hls_only.m3u', 'playztv_hls_all.m3u', 'playztv_master.m3u', 'playztv_live.m3u', 'playztv_live_all.m3u', 'playztv_upcoming.m3u', 'playztv_upcoming_all.m3u', 'status.json', 'events.json', 'playztv_streams.csv']:
